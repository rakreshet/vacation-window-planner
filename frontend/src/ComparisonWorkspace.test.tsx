...
test('end-date selection starts at the chosen start date and allows a one-day break', () => {
  render(<Harness />)
  const start = screen.getByLabelText('Start date')
  const end = screen.getByLabelText('End date')
  expect(end).toHaveAttribute('min', '2027-01-03')
  fireEvent.change(end, { target: { value: '2027-01-02' } })
  expect(end).toBeInvalid()
  fireEvent.change(end, { target: { value: '2027-01-03' } })
...
test('moving the start past an existing end explains the problem before comparison', () => {
  ...
  const end = screen.getByLabelText('End date')
  expect(end).toHaveValue('2027-01-07')
  expect(end).toHaveAccessibleDescription('Choose an end date on or after the start date.')
  expect(screen.getByRole('alert')).toHaveTextContent(
    'Choose an end date on or after the start date.',
  )
  expect(screen.getByRole('button', { name: 'Compare dates' })).toBeDisabled()
  expect(screen.getByRole('button', { name: 'Move 1 day earlier' })).toBeDisabled()
  expect(screen.getByRole('button', { name: 'Move 1 day later' })).toBeDisabled()
  expect(fetch).not.toHaveBeenCalled()
  fireEvent.change(end, { target: { value: '2027-01-10' } })
